...
\begin{document}
	...
	The pull-down circuit of the NAND gate is composed of two NMOS transistors in series. The pull-up circuit is the complement of the pull-down circuit and is composed of two parallel PMOS transistors. We can create an equivalent inverter from the NAND gate. In this equivalent inverter, the NMOS channel length is doubled because the NMOS transistors in the NAND gate are laid out in series. Similarly, when both PMOS gates in the NAND gate are on, the effective resistance of the pullup network is halved. This implies the width of the effective PMOS gate is halved. However, in the case for the PMOS gate in the   Note that we also increase the width of the PMOS transistor by a factor of 2 to account for the lower mobility of holes in the p-type material. The schematic for the resulting circuit is shown in Figure \ref{fig::nand_schematic}.
