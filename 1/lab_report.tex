...
\begin{document}
	...
	\noindent For our test setup, $\alpha = 1/2$ and $f = 1/(20 ns)$. For the capacitance, we need to consider the extrinsic intrinsic and instrinsic switching capacitances. The extrinsic capacitance is configured to be $4 fF$, and the intrinsic capacitance can be estimated by:
	 ...
	 \end{equation}
	  ...
	  \noindent where $C_{gp}$ and $C_{gn}$ are estimated using Table \ref{table::nand_gate_eq_params}. Letting $C_{gp} = 1.9 fF$ and $C_{gn} = 1.6 fF$, we find that $C_{int} \approx 8.6 fF$. Substituting, we find $P_total \approx 1.021 {\mu}W$. Note that this estimate is pretty close to the measured value and could be improved by considering the contribution of $P_{\text{static}}$ and $P_{\text{shortcircuit}}$.
