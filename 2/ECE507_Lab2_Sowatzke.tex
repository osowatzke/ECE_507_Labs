...
\begin{document}
	...
	\noindent Similar to Figure \ref{fig::nand_delay_analysis_test_circuit}, we find that the input \texttt{A} is also being driven by a pulsed input source, while the input \texttt{B} is tied to \texttt{VDD}. The pulsed input source has a slightly larger period, which results in a slightly longer simulation time. The resulting power consumption is included in Figure \ref{fig::nand_power_analysis}.
